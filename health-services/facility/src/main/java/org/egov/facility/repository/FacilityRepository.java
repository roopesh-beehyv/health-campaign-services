...
public class FacilityRepository extends GenericRepository<Facility> {
    ...
    public List<Facility> findById(List<String> ids, String columnName, Boolean includeDeleted) {
        ...
        String query = String.format("SELECT *, a.id as addressId, a.tenantid as addressTenantId FROM facility f LEFT JOIN address a ON f.addressid = a.id WHERE f.%s IN (:ids) AND isDeleted = false", columnName);
        if (null != includeDeleted && includeDeleted) {
            query = String.format("SELECT *, a.id as addressId, a.tenantid as addressTenantId FROM facility f LEFT JOIN address a ON f.addressid = a.id  WHERE f.%s IN (:ids)", columnName);
        }
    ...
    public List<Facility> find(FacilitySearch searchObject, Integer limit, Integer offset, String tenantId, Long lastChangedSince, Boolean includeDeleted) throws QueryBuilderException {
        String query = "SELECT *, a.id as addressId, a.tenantid as addressTenantId FROM facility f LEFT JOIN address a ON f.addressid = a.id";
